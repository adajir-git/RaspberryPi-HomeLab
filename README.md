...
> `node_exporter` runs as a native systemd service (`prometheus-node-exporter`) directly on the host — not in Docker — so it can access hardware metrics without containerization overhead. It scrapes custom `.prom` files from `/var/lib/node_exporter/textfile_collector/` generated by the scripts in `scripts/`.
